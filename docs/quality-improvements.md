...
| Variants | UI restores installed choices. Nonempty catalogs reject empty selections and unknown IDs; the UI already disabled an empty selection, so backend behavior now agrees. Store sorted, deduplicated requested IDs. Legacy scheduled records with empty IDs resolve defaults once. |
