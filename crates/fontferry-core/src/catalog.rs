...
impl FontDefinition {
    pub fn validate(&self) -> Result<()> {
        ...
        if self.delivery_policy == DeliveryPolicy::AutoInstall && self.artifact_provider.is_none() {
            return Err(FontFerryError::InvalidCatalog(format!(
...
mod optional_date {
    ...
    where
        S: Serializer,
    {
        value.map(|date| date.to_string()).serialize(serializer)
    }
