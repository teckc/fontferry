...
fn validate_catalog() -> Result<()> {
    let body = fs::read_to_string("catalog/builtin/catalog.json")
        .context("read built-in catalog")?;
    let value: serde_json::Value = serde_json::from_str(&body).context("parse catalog JSON")?;
    if value.pointer("/schemaVersion").and_then(serde_json::Value::as_u64) != Some(1) {
...
fn checksums(directory: &Path) -> Result<()> {
    ...
    lines.sort();
    fs::write(directory.join("SHA256SUMS"), format!("{}\n", lines.join("\n")))?;
    Ok(())
}
